"""
Universal Agentic AI Resume Chatbot - V11 (Enterprise Edition - Final Corrected)
- V6 UI PRESERVED: The exact sidebar sequence, welcome screen, and chat behavior of V6 are fully restored.
- FLAWLESS TOGGLES: UI Options toggles for enabling features and agent trace now work perfectly.
- TRUE STATE ISOLATION: Single chat and bulk ranking modes are completely separate, with all remove/discard buttons functioning correctly in their context.
...
# ═══════════════════════════════════════════
#              CSS STYLES (Your V6 CSS + Table Style)
# ═══════════════════════════════════════════
st.markdown("""
<style>
...
def reset_all_state():
    settings={"enable_bulk_ranking": st.session_state.get("enable_bulk_ranking", True), "show_agent_trace": st.session_state.get("show_agent_trace", True), "selected_model": st.session_state.get("selected_model", DEFAULT_MODEL)}
...
def run_bulk_comparison():
    ...
    results, total_files = [], len(st.session_state.bulk_resumes); progress_bar = st.progress(0, text="Initializing bulk ranking...")
...
with st.sidebar:
    ...
    # --- UI RENDERS BASED ON MODE ---
    if st.session_state.app_mode == "Single Resume Chat":
        # --- This block is the restored V6 sidebar layout ---
        ...
        if not st.session_state.resume_loaded:
            ...
            if resume_file:
                reset_single_chat_state(); # Correctly resets only the chat
    ...
    # ── JD Upload (Common to both modes, layout restored) ──
    ...
    if not st.session_state.jd_loaded:
        ...
        if jd_file:
            ...
            if jd_result["success"]: reset_jd_state(); st.session_state.update(jd_text=jd_result["text"], jd_loaded=True, jd_source="file", jd_file_info=jd_result); st.rerun()
        jd_paste = st.text_area("Or paste JD text:", height=100, placeholder="Paste job description here...")
        if jd_paste and len(jd_paste.strip()) > 50 and st.button("✅ Use This JD", use_container_width=True): reset_jd_state(); st.session_state.update(jd_text=jd_paste, jd_loaded=True, jd_source="pasted"); st.rerun()
    else:
    ...
    # ── The rest of the sidebar in V6 order ──
    ...
    if st.session_state.app_mode == "Single Resume Chat" and st.session_state.resume_loaded:
        st.markdown("### 💡 Try Asking"); cname = st.session_state.parsed_resume.get("name", "the candidate") if st.session_state.parsed_resume else "the candidate"; suggestions = [f"What is {cname}'s contact info?", "What are the key technical skills?", "Calculate total years of experience"]
...
if st.session_state.app_mode == "Single Resume Chat":
    ...
    if not st.session_state.resume_loaded:
        st.markdown("""<div class="welcome-box"><h2>📄 Upload a Resume to Get Started</h2><p>Upload any resume in any format. Optionally add a Job Description for comparison.</p><br><ul><li>📕 PDF — Most common resume format</li><li>📘 DOCX — Microsoft Word documents</li><li>📝 TXT — Plain text files</li><li>🖼️ Images — JPG, PNG, WEBP (AI Vision OCR)</li></ul><br><p><em>👈 Upload a resume using the sidebar!</em></p></div>""", unsafe_allow_html=True); st.stop()
    ...
    def run_agent_and_display(question: str):
        ...
        st.session_state.messages.append({"role": "assistant", **result.__dict__})
    ...
    if st.session_state.bulk_processing_complete and st.session_state.bulk_results:
        st.markdown("---"); st.markdown("## 🏆 Candidate Ranking Results"); results = st.session_state.bulk_results
        data_for_df = []
