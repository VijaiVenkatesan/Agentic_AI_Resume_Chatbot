...
from document_processor import extract_contacts_from_text, extract_name_from_text
...
def _merge_contacts(llm_parsed: Dict, regex_contacts: Dict, doc_contacts: Dict) -> Dict:
    """
    ...
    for field in contact_fields:
        ...
        llm_val = llm_parsed.get(field, "").strip()
        doc_val = doc_contacts.get(field, "").strip()
        regex_val = regex_contacts.get(field, "").strip()
        
        # Priority: LLM > Document > Regex (but validate LLM)
...
def _is_valid_field(field: str, value: str) -> bool:
    ...
    invalid_values = ['n/a', 'na', 'none', 'not available', 'not specified', 
                      'unknown', 'null', '-', '—', 'candidate', 'your name',
                      'email@example.com', 'xxx', '000']
    ...
    if field == 'name':
        # Name should have at least 2 words and be mostly alphabetic
        words = value.split()
        if len(words) < 2:
            return False
        alpha_ratio = sum(1 for c in value if c.isalpha() or c.isspace()) / len(value)
        return alpha_ratio > 0.8
    
    return True


# Update the parse_resume_with_llm function to use document contacts:

def parse_resume_with_llm(
    resume_text: str,
    groq_api_key: str,
    model_id: str = "llama-3.1-8b-instant",
    doc_contacts: Dict = None  # NEW: Pass pre-extracted contacts
) -> Dict:
        ...
        "messages": [
            {
                ...
                "content": (
                    ...
                    "Extract EVERY piece of information, especially contact details."
                )
            },
            {
                "role": "user",
    ...
    # Step 3: ENHANCED - Merge contacts from all sources
    ...
    if not parsed.get("name") or not _is_valid_field("name", parsed.get("name", "")):
        ...
        name_candidates = [
            ...
            regex_name,
            extract_name_from_text(resume_text)  # Try again with enhanced function
        ]
        ...
        if not parsed.get("name") or parsed["name"] in ["", "Unknown", "N/A", "Candidate"]:
            parsed["name"] = "Candidate"
